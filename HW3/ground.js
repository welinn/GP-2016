...
function makeGround(){
  ...
  var mesh4 = new THREE.Mesh(new THREE.PlaneGeometry((tenTheta + 1) * totalLen * 2 / 15, window.innerHeight * 0.3), met);
  var mesh5 = new THREE.Mesh(new THREE.PlaneGeometry(totalLen / 15, totalLen / 75 + window.innerHeight * 0.1), met);
  ...
  mesh4.position.set(totalLen / 3, -window.innerHeight * 0.15, 0);
  mesh4.rotation.z = theta;
  mesh4.position.y += tenTheta * totalLen / 15;
  mesh5.position.set(totalLen * 13 / 30, totalLen / 150 - window.innerHeight * 0.05, 0);
  var mesh6 = mesh4.clone();
  var mesh7 = mesh2.clone();
  var mesh8 = mesh1.clone();
  mesh6.rotation.z = -theta;
  mesh6.position.x = totalLen * 8 / 15;
